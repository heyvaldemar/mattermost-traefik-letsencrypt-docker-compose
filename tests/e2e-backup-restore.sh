#!/bin/bash
# End-to-end tests for the mattermost-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-mattermost-traefik-letsencrypt-docker-compose.yml}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./mattermost-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./mattermost-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./mattermost-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./mattermost-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
