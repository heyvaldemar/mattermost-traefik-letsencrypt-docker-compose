#!/usr/bin/env bash
# mattermost-restore-database.sh [backup-file-name]
#
# Replaces the Mattermost database with one of the dumps the backups service wrote.
#
#   ./mattermost-restore-database.sh               list and ask
#   ./mattermost-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than mattermost.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-mattermost}"
APP_SERVICE="mattermost"
...
DB_NAME="$(env_of MATTERMOST_DB_NAME)"; DB_USER="$(env_of MATTERMOST_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Mattermost." >&2
  exit 1
fi
