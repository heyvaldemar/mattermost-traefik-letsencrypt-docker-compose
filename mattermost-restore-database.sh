...
MATTERMOST_CONTAINER=$(docker ps -aqf "name=mattermost-mattermost")
...
for entry in $(docker container exec "$MATTERMOST_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker stop "$MATTERMOST_CONTAINER"

echo "--> Restoring database..."
docker exec "$MATTERMOST_BACKUPS_CONTAINER" sh -c "dropdb -h postgres -p 5432 $MATTERMOST_DB_NAME -U $MATTERMOST_DB_USER \
&& createdb -h postgres -p 5432 $MATTERMOST_DB_NAME -U $MATTERMOST_DB_USER \
&& gunzip -c ${BACKUP_PATH}${SELECTED_DATABASE_BACKUP} | psql -h postgres -p 5432 $MATTERMOST_DB_NAME -U $MATTERMOST_DB_USER"
echo "--> Database recovery completed..."

echo "--> Starting service..."
docker start "$MATTERMOST_CONTAINER"
