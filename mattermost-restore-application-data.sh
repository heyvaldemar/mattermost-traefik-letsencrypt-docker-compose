...
BACKUP_PATH="/srv/mattermost-application-data/backups/"
RESTORE_PATH="/mattermost/data/"
BACKUP_PREFIX="mattermost-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$MATTERMOST_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$MATTERMOST_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
