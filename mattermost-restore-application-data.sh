#!/usr/bin/env bash
# mattermost-restore-application-data.sh [backup-file-name]
#
# Replaces Mattermost's application data with one of the archives the backups
# service wrote.
#
#   ./mattermost-restore-application-data.sh               list and ask
#   ./mattermost-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Mattermost." >&2
  exit 1
fi
